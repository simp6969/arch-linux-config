...
apply_video_wallpaper() {
  ...
  # Extract thumbnail for color generation
  local video_name=$(basename "$video_path")
  local cache_preview_image="$HOME/.cache/video_preview/${video_name}.png"
  
  if [[ ! -f "$cache_preview_image" ]]; then
    mkdir -p "$HOME/.cache/video_preview"
    ffmpeg -v error -y -i "$video_path" -ss 00:00:01.000 -vframes 1 "$cache_preview_image"
  fi

  # Generate colors and refresh
  "$SCRIPTSDIR/WallustSwww.sh" "$cache_preview_image"
  sleep 2
  "$SCRIPTSDIR/Refresh.sh"
}
